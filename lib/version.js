    ...
    filesList.forEach(function(file, index) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (options.analyze) {
                ...
                if (match != null && match[1] != versionToReplace) {
                    wrongVersions ++;

                    if (options.verbose) {
                        wrongLines.push(lines[i].replace(match[1], customSeparator + match[1] + customSeparator));
                    }

                    if (options.fix) {
                        versionToReplace = match[1];
                    }
                }
            }
        ...
        if (options.analyze) {
            if (options.verbose) {
                wrongLines.forEach(function(line) {
                    line = line.split(customSeparator);

                    if (options.fix) {
                        console.log(spacer22 + chalk.black.bgYellow(line[0]) + chalk.white.bold.bgRed(line[1]) + chalk.white.bold.bgGreen(replaced) + chalk.black.bgYellow(line[2]));
                    }
                    else {
                        console.log(spacer22 + chalk.black.bgYellow(line[0]) + chalk.white.bold.bgRed(line[1]) + chalk.black.bgYellow(line[2]));
                    }
                });
            }
            if (wrongVersions > 0) {
                if (options.fix) {
                    console.log(spacer22 + chalk.white.bgGreen.bold('fixed ' + wrongVersions + ' wrong version numbers in this file'));
                }
                else {
                    console.log(spacer22 + chalk.white.bgRed.bold('found ' + wrongVersions + ' wrong version numbers in this file'));
                }
            }
        }
    });
