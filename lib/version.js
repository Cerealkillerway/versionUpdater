...
var Errors = {
    ...
    "E002": {
        description: "Configuration file (versionFilesList.json) is from an older and no more compatible version of VersionUpdater",
        text: "It seems that your versionFilesList.json comes from an old and no more compatible version of VersionUpdater\n" + spacer15 + "It is required to re-init the folder (version init -f)"
    }
};
...
// delete files list
function revertInit(reinit, list, prefix) {
...
function loadConfiguration() {
    ...
    if (isInit()) {
        ...
        if (!configuration.versionPrefix) {
            sendError("E002");
        }
    }
...
function updateFilesList(adds, removes, prefix) {
...
function updateFiles(configuration, newVersion) {
    ...
    filesList.forEach(function(file, index) {
        ...
        print("processed file: " + file + " - made " + fileOccurencies + " " + replacementWord, 'completed', 'date');
    });
...
program
    ...
    .action(function(options) {
        ...
        if (isInit()) {
            if (options.force) {
                revertInit(true, options.add, options.prefix);
            }
            else {
                if (options.add || options.remove || options.prefix) {
                    updateFilesList(options.add, options.remove, options.prefix);
                }
                else {
                    sendWarning('W001');
                }
        ...
        // if not initialized before, init now
            ...
            init(options.add, options.prefix);
        }
    });
...
program
    ...
    .action(function(options) {
        ...
        // in this case will not use loadConfiguration(), since filesList can be displayed correctly
        // also from old versions of configuration file.
        ...
        configuration.filesList.forEach(function(file, index) {
            print((index + 1) + " - " + file, "important", "spaced22");
            filesCounter++;
        });
