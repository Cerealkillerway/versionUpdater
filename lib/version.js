...
function updateFiles(configuration, newVersion, analyze, verbose) {
    let occurencies = 0;
    let filesList = configuration.filesList;
    let oldVersion = configuration.currentVersion;
    let vOldVersion = configuration.versionPrefix + oldVersion;
    let anyVersionRegex = {
    ...
    filesList.forEach(function(file, index) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (analyze) {
                ...
                if (match != null && match[0] != versionToReplace) {
                    wrongVersions ++;

                    if (verbose) {
                        wrongLines.push(lines[i]);
                    }
                }
            }
        ...
        if (analyze) {
            if (verbose) {
                wrongLines.forEach(function(line) {
                    print(line, 'warning', 'spaced22');
                });
            }
            if (wrongVersions > 0) {
                print('found ' + wrongVersions + ' wrong version numbers in this file', 'warning', 'spaced22');
            }
        }
    });
...
program
    ...
    .action(function(newVersion, options) {
        ...
        updateFiles(configuration, newVersion, options.analyze, options.verbose);
        console.log('');
        process.exit(0);
    });
