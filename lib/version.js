        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (options.analyze) {
                ...
                if (match != null && match[1] != versionToReplace) {
                    ...
                    if (options.verbose) {
                        wrongLines.push(lines[i].replace(match[1], customSeparator + (i + 1).toString() + customSeparator + match[1] + customSeparator));
                    }
        ...
        if (options.analyze) {
            if (options.verbose) {
                wrongLines.forEach(function(line) {
                    ...
                    if (options.fix) {
                        console.log(spacer22 + chalk.black.bgYellow('[Line: ' + line[1] + ']   ' + line[0]) + chalk.white.bold.bgRed(line[2]) + chalk.white.bold.bgGreen(replaced) + chalk.black.bgYellow(line[3]));
                    }
                    else {
                        console.log(spacer22 + chalk.black.bgYellow('[Line: ' + line[1] + ']   ' + line[0]) + chalk.white.bold.bgRed(line[2]) + chalk.black.bgYellow(line[3]));
                    }
                });
            }
            if (wrongVersions > 0) {
